...
const addChapter = async (req,res) => {
    try{
        ...
        let extChapter = await Chapter.findOne({ title })
            ...
            res.status(StatusCodes.CREATED).json({ status: true, msg: "chapter created successfully", chapter: newChapter })
    }catch (err){
    ...
    try{
        ...
        let extChapter = await Chapter.findById(id)
         ...
         fs.unlink(filePath, async (err) => {
            ...
            //update in chapter collections
            await Chapter.findByIdAndUpdate({_id: chId })
